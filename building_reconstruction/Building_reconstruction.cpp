...
PolygonMesh urban_rec::Building_reconstruction::filterMeshByMesh(PolygonMesh mesh_input, PolygonMesh mesh_filter) {
    ...
    for (std::vector<Vertices>::iterator it1 = mesh_input.polygons.begin(); it1 != mesh_input.polygons.end(); it1++) {
        ...
        for (std::vector<Vertices>::iterator it2 = mesh_filter.polygons.begin();
            ...
            if (!pcenter_accepted) {
                double s2 = Geometry_pcl::triangle_area_geron(pcenter, p1_sr, p2_sr) +
                            Geometry_pcl::triangle_area_geron(pcenter, p2_sr, p3_sr) +
                            Geometry_pcl::triangle_area_geron(pcenter, p1_sr, p3_sr);
...
PolygonMesh urban_rec::Building_reconstruction::filterMeshPoissonByPointsNoExtra(PolygonMesh mesh_input,
    ...
    do {
        ...
        for (std::vector<Vertices>::iterator it = new_polygons_filtering.begin();
             it != new_polygons_filtering.end(); it++) {
            Indices vertecies = it->vertices;
            PointXYZ p1;
            PointXYZ p2;
            PointXYZ p3;
    ...
    for (PointXYZ p_repeat: *xyz_cloud_repeat) {
        ...
        for (PointXYZ p_ideal: *xyz_cloud_ideal) {
            if (min_dist_p_repeat < abs(p_repeat.x - p_ideal.x) || min_dist_p_repeat < abs(p_repeat.y - p_ideal.y)
                || min_dist_p_repeat < abs(p_repeat.z - p_ideal.z))
                continue;
    ...
    for (PointXYZ p_ideal: *xyz_cloud_ideal) {
        ...
        for (PointXYZ p_repeat: *xyz_cloud_repeat) {
            if (min_dist_p_ideal < abs(p_repeat.x - p_ideal.x) || min_dist_p_ideal < abs(p_repeat.y - p_ideal.y)
                || min_dist_p_ideal < abs(p_repeat.z - p_ideal.z))
                continue;
...
PolygonMesh urban_rec::Building_reconstruction::reconstruct2() {
    ...
    for (PointXYZ p: *all_vertices) {
        ...
        for (std::vector<Vertices>::iterator it2 = upsample_hull_mesh.polygons.begin();
            ...
            if (!p_accepted) {
                double s2 = Geometry_pcl::triangle_area_geron(p, p1_sr, p2_sr) +
                            Geometry_pcl::triangle_area_geron(p, p2_sr, p3_sr) +
                            Geometry_pcl::triangle_area_geron(p, p1_sr, p3_sr);
                if (s2 < s1 * coefficient_distance_filtering) {
                    p_accepted = true;
                }
            } else {
